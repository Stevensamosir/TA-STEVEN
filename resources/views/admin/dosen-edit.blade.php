        ...
        <form action="{{ route('admin.dosen.update', $lecturer->id) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $lecturer->user->name) }}" required
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-del/30">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Email</label>
                    <input type="email" name="email" value="{{ old('email', $lecturer->user->email) }}" required
                           class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-del/30">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Role</label>
                    {{-- Halaman ini Dekan-only, semua opsi role tersedia --}}
                    <select name="role" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none">
                        @foreach(['dosen', 'kaprodi', 'dekan'] as $r)
                            <option value="{{ $r }}" {{ $lecturer->user->role === $r ? 'selected' : '' }}>
                                {{ ucfirst($r) }}
                            </option>
                        @endforeach
                    </select>
                    @if(!$dekanExists)
                        <p class="text-xs text-amber-500 mt-1">⚠ Belum ada Dekan aktif lain.</p>
                    @endif
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="expertise" rows="2"
                          class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none resize-none">{{ old('expertise', $lecturer->expertise) }}</textarea>
            </div>
